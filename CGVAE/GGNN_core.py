...
import pickle
import random
import CGVAE.utils as utils
from CGVAE.utils import MLP, dataset_info, ThreadedIterator, graph_to_adj_mat, SMALL_NUMBER, LARGE_NUMBER, graph_to_adj_mat
...
class ChemModel(object):
    ...
    def transform(self, X, z_mean):
        #mapping data to the latent space. X is a batch of inputs

        return self.sess.run(z_mean, feed_dict={self.x: X})
